...
type FieldsMap = { [k: string]: FieldState };

type SubscriptionFormProps = {
  [k: string]: any;
};

export type StatusConditions = "success" | "failure" | "initial";
...
> {
  ...
  initializeState = (): FieldsMap => {
    ...
    const newState: { [k: string]: FieldState } = {};
    ...
    return newState as FieldsMap;
  };

  generatePostRequest = () => {
    const stateFields = this.state.fields;
    const requestObject: { [k: string]: any } = {};
  ...
  isValid = async () => {
    ...
    for (const field in fields) {
      ...
      if (isField) {
        ...
        // isn't valid!
        if (element.missing || element.invalid) return false;
      }
    }
  ...
  updateAllFields = (): FieldsMap => {
    // update basic state
    const { fields } = this.state;
    const newFields: { [k: string]: FieldState } = {};
    for (const field in fields) {
      const [isField, fieldName] = validFieldName(field);
      if (isField) {
        const element = fields[fieldName];
    ...
    return newFields as FieldsMap;
  };

  changeHandler = (field: string, event: React.ChangeEvent<TargetElements>) => {
        ...
        {fieldKeys.map((fieldName, idx) => {
          ...
          return (
            <Field
              ...
              {...toPass}
              state={state}
              changeHandler={e => this.changeHandler.call(this, fieldName, e)}
              blurHandler={e => this.blurHandler.call(this, fieldName)}
            />
          );
        }, this)}
      </FormLayout>
